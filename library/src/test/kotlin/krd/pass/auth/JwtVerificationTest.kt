...
class JwtVerificationTest {
    ...
    fun `a correctly signed token with matching iss aud and a future exp is accepted`() {
        ...
        val claims = KrdpassAuth.verifyJwt(token, jwkSource, issuer, audience, 60)
    ...
    fun `validateIdToken accepts a fully valid token whose nonce matches`() {
        ...
        // Should not throw.
        KrdpassAuth.validateIdTokenWithSource(token, jwkSource, issuer, audience, "nonce-123")
    }
    ...
    fun `validateIdToken rejects a token whose nonce does not match (replay)`() {
        ...
        assertAuthFailed("nonce") {
            KrdpassAuth.validateIdTokenWithSource(token, jwkSource, issuer, audience, "expected-nonce")
        }
    }
    ...
    fun `validateIdToken rejects a null id_token`() {
        assertAuthFailed("id_token") {
            KrdpassAuth.validateIdTokenWithSource(null, jwkSource, issuer, audience, "n")
        }
    }
    ...
    private fun assertRejected(token: String) {
        try {
            KrdpassAuth.verifyJwt(token, jwkSource, issuer, audience, 60)
